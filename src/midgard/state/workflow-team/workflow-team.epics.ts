...
export class WorkflowTeamEpics {
   ...
   */
  loadAllWorflowTeamsEpic = (action$: Observable<any>) => action$.pipe(
    reduxObservable.ofType(LOAD_ALL_WORKFLOWTEAMS),
    switchMap(() => this.httpService.makeRequest('get', `${environment.API_URL}/workflowteam/`, null, true).pipe(
      // If successful, dispatch success action with result
      map((res: Action) => loadWorflowTeamsCommit(res.data)),
   ...
   * @param {Observable} action$ - the current action
   */
  loadOneWorflowTeamEpic = (action$: Observable<any>) => action$.pipe(
    reduxObservable.ofType(LOAD_ONE_WORKFLOWTEAM),
    switchMap((action: Action) => this.httpService.makeRequest('get', `${environment.API_URL}/workflowteam/${action.id}/`, null, true).pipe(
      // If successful, dispatch success action with result
   ...
   * @param {Observable} action$ - the current action
   */
  createWorflowTeamEpic = (action$: Observable<any>) => action$.pipe(
    reduxObservable.ofType(CREATE_WORKFLOWTEAM),
    switchMap((action: Action) => this.httpService.makeRequest('post', `${environment.API_URL}/workflowteam/`, action.data, true).pipe(
      // If successful, dispatch success action with result
   ...
   * @param {Observable} action$ - the current action
   */
  updateWorflowTeamEpic = (action$: Observable<any>) => action$.pipe(
    reduxObservable.ofType(UPDATE_WORKFLOWTEAM),
    switchMap((action: Action) => this.httpService.makeRequest('put', `${environment.API_URL}/workflowteam/${action.data.id}/`, action.data, true).pipe(
      // If successful, dispatch success action with result
   ...
   * @param {Observable} action$ - the current action
   */
  deleteWorflowTeamEpic = (action$: Observable<any>) => action$.pipe(
    reduxObservable.ofType(DELETE_WORKFLOWTEAM),
    switchMap((action: Action) => this.httpService.makeRequest('delete', `${environment.API_URL}/workflowteam/${action.data.id}/`, null, true).pipe(
      // If successful, dispatch success action with result
      map(() => deleteWorflowTeamCommit(action.data, action.nested)),
      // If request fails, dispatch failed action
